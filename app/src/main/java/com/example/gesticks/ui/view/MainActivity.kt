...
class MainActivity : AppCompatActivity() {
    ...
    private fun forceInitialWearSync() {
        val session = SessionManager(this)
        val token = session.fetchAuthToken()
        val userId = session.fetchUserId()
        if (token != null) {
            // Enviamos una sincronización vacía solo para pasar el token y userId de inmediato
            wearSyncManager.syncTickets(emptyList(), token, userId)
        }
    }
